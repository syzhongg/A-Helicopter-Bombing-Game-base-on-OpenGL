...
public:
	My_Mesh();
	~My_Mesh();
	void load_obj(std::string obj_File, mat4 transform);
	...
	void generate_floor(float height, float x, float y, float dx, float dy, int num);
	void generate_wall(float height, float x, float y, float dx, float dy, int num);
	void generate_body();
	...
	void set_translatexz(float x, float z);
	vec3 Opoints;
...
public:
	...
	bool isShadow();
	void setball(bool flag);
	bool isball();
	void set_theta_step(float x, float y, float z);
	void get_theta_step(float& x, float& y, float& z);
	void set_theta345(float x, float y, float z);
